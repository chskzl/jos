...
sys_exofork(void)
{
	...
	struct Env * e;
    int errorcode = env_alloc(&e, curenv ? curenv->env_id: 0);
    if (errorcode < 0) {
        return errorcode;
    }
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if (((perm & (PTE_P | PTE_U)) != (PTE_P | PTE_U)) || ((perm & 0xfff) & (~PTE_SYSCALL)))
        return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    pte_t * p_pte;
    struct PageInfo * pp = page_lookup(srcenv->env_pgdir, srcva, &p_pte);
    if ((perm & PTE_W) && (*p_pte & PTE_W) != PTE_W)
        return -E_INVAL;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
    ...
    case SYS_env_set_pgfault_upcall:
        return sys_env_set_pgfault_upcall((envid_t) 1, (void *) a2);
